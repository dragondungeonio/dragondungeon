...
export class ArenaRoom extends CoreRoom {
    constructor() {
        ...
        state.coinJars.set(
            v4(),
            new CoinJar(1500, 1500, 0),
        )

        let botPlayerA = new Player('mud', 0, 0)
        botPlayerA.onlineName = botNames[Math.floor(Math.random() * botNames.length)]
        botPlayerA.isBot = true
        state.players.set(v4(), botPlayerA)

        let botPlayerB = new Player('poison', 0, 0)
        botPlayerB.onlineName = botNames[Math.floor(Math.random() * botNames.length)]
    ...
    tick(): void {
        super.tick()
        for (let i = super.getState().coins.size; i < super.getState().players.size * 20; i++) {
            super.spawnCoin()
        }
    }
}
