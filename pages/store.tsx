...
function GemStoreItem(props) {
  ...
  return <div className={styles.borderSliceApplied} style={{ border: `5px solid ${props.isRealMoneyPurchase ? '#522398' : 'whitesmoke'}`, padding: '20px', width: '200px', background: 'black' }} onClick={() => {
    if (props.owned) {
      ...
      if (!props.isRealMoneyPurchase) {
        if (props.cost <= props.player) {
          if (confirm(`You are about to buy ${props.name} for ${props.cost} Gems.`)) {
            fetch(`${window.location.protocol}//${window.location.hostname}:1337/purchase/${props.id}?user=${props.token}`)
              .then((r) => {
                if (r.status == 200) {
    ...
    {!props.owned && <><span style={{ color: affordable ? 'white' : 'red', fontSize: '15pt' }}>{props.cost}</span>{props.isRealMoneyPurchase ? '' : ' Gems'}</>}
    {props.owned && <span style={{ color: 'green', fontSize: '15pt' }}>Owned</span>}
    <br /><p>{props.description}</p><p style={{ color: '#f9e300', fontSize: '10pt' }}>{props.perk}</p>
  </div>
}
...
export default function MyDragon() {
  ...
  return (
    <div className={styles.pageContent}>
      ...
      {skinsLoaded && <GemStoreSection>
        {skins.map(skin => {
          return <GemStoreItem isRealMoneyPurchase={false} token={token} name={skin.name} id={skin.id} owned={entitledSkins.includes(skin.id)} description={skin.description} img={skin.thumbnail} cost={skin.gemCost} perk={skin.perk} player={gems} />
        })}
      </GemStoreSection>}
      <br /><br />
    </div>
